...
def perform_basic_installation():
    ...
    subprocess.run(["sudo", "apt", "install", "-y", "vim", "nano", "curl", "wget", "git", "htop"])
...
def install_wine_winetricks():
    ...
    subprocess.run(["sudo", "apt", "update"])
    subprocess.run(["sudo", "apt", "install", "-y", "--install-recommends", "winehq-stable"], check=False)
    subprocess.run(["sudo", "apt", "install", "-y", "winetricks"])

def install_python_packages():
    subprocess.run(["sudo", "apt", "update"])
    subprocess.run(["sudo", "apt", "install", "-y", "git", "python3", "python3-pip", "python3-virtualenv", "build-essential"])
...
def amdgpu_rocm_setup():
    with open("/etc/apt/sources.list.d/rocm.list", "w") as f:
        f.write("deb [arch=amd64] https://repo.radeon.com/rocm/apt/debian/ ubuntu main\n")
    subprocess.run(["sudo", "apt", "update"])
    subprocess.run(["sudo", "apt", "install", "-y", "rocm-dkms"])
    subprocess.run(["sudo", "usermod", "-a", "-G", "video", os.getenv("SUDO_USER", os.getlogin())])
    subprocess.run(["sudo", "usermod", "-a", "-G", "render", os.getenv("SUDO_USER", os.getlogin())])

def nvidia_gpu_setup():
    driver = input("Enter the recommended NVIDIA driver version (e.g., nvidia-driver-XXX): ")
    subprocess.run(["sudo", "apt", "install", "-y", driver])
